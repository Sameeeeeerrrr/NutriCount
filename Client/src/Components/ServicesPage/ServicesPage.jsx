...
export function ServicesPage() {
    ...
    useEffect(() => {
        async function checkin() {
            try {
                const response = await axios.get(`${LINK}/api/auth?token=${token}`)
    ...
    const sendReq = async () => {
        if (food == "") {
            ...
            try {
                ...
                const response = await axios.post(`${LINK}/api/find?token=${token}`, data)
    ...
    const deleteReq = async () => {
        ...
        if (food) {
            try {
                const response = await axios.post(`${LINK}/api/remove?token=${token}&username=${localStorage.getItem("username")}`, data)
                console.log(response)
        ...
        alignItems: 'center',
            ...
            <div className="servicebox">
                ...
                <div className="searchbar">
                    ...
                    <button className="findbtn" onClick={sendReq}>Search</button>
                    <button className="deletebtn" onClick={deleteReq}>Delete</button>
                </div>
...
function Viewallitems() {
    ...
    useEffect(() => {
        async function run() {
            const token = localStorage.getItem("token")
            const response = await axios.get(`${LINK}/api/all?token=${token}`)
            const output = response.data.fooditem
            setItems(output)
            console.log(response)
        }
